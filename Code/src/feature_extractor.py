...
def get_feature_vectors(image, name, output_dim, sub_features_dir, hog_dict, elbp_dict={"neighbours":8, "radius":1},feature_visualization=False):
...
def save_all_img_features(images, output_dim, sub_features_dir, hog_dict, feature_visualization = False, img_ids=None):
  if img_ids!=None:
  ...
  for i in range(images.shape[0]):  
    ...
    color_moments, elbp_out, hog_out = get_feature_vectors(images[i], i, output_dim, sub_features_dir, hog_dict, feature_visualization=False)
  ...
  pd.DataFrame(color_moments_lst).to_csv(os.path.join(features_dir, "color_moments.csv"),index=False)
