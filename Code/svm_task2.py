...
def svm_task_2(training_features_dir, test_features_dir, training_set_features = None, test_set_features = None):
    ...
    for original_sub_id in subject_ids_list:
        count = count + 1
        ...
        for image_id in range(len(training_set_features)):
            ...
            if training_sub_ids[image_id] == original_sub_id:
        ...
        for image_id in testing_ids:
            if predictions[cnt] == 1:
                if image_id in initial_values:
                    if any(original_sub_id in type for type in initial_values[image_id]):
                        for value in initial_values[image_id]:
                            if original_sub_id in value:
                                value[original_sub_id] = max(value[original_sub_id], output_pred[cnt])
                    else:
                        y = {original_sub_id: output_pred[cnt]}
                        initial_values[image_id].append(y)
                else:
                    x = {original_sub_id: output_pred[cnt]}
                    initial_values[image_id] = [x]
            else:
                if image_id in negative_values:
                    if any(original_sub_id in type for type in negative_values[image_id]):
    ...
    jsonFile = open("Values_Task2.json","w")
    ...
    jsonFile = open("Negative_Values_Task2.json","w")
    jsonFile.write(jsonString)
    jsonFile.close()

    jsonString = json.dumps(results, cls=NumpyEncoder, indent=4)
    jsonFile = open("Results_Task2.json","w")
    jsonFile.write(jsonString)
    jsonFile.close()
    ...
    predicted_val = []
    for key in results:
        type_img = get_sub_from_image_path(key)
